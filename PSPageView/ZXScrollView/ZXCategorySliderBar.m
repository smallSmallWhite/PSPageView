...
- (void)commonInitWithStyle:(ZXPageStyle *)style{
    self.style = style;
    _originIndex = 0;
    _currentIndex = 0;
...
- (void)setSelectIndex:(NSInteger)index{
    ...
    [self adjustContentOffsetToIndex:index completeHanle:^{
        ...
        itemView.contentLabel.textColor = self.style.titleSelectedColor;
        NSNotification *notification = [[NSNotification alloc]initWithName:RESETCOLORNOTIFICATION object:@"color" userInfo:@{@"color":self.style.titleColor,@"index":@(self.currentIndex)}];
...
- (UIView *)indicateView
{
    if (!_indicateView) {
        ...
        _indicateView = [[UIView alloc]initWithFrame:CGRectMake([self.itemOriginXArray[_originIndex] floatValue], self.style.titleViewHeight - self.style.bottomLineHeight, [self.itemWidthArray [_originIndex] floatValue], self.style.bottomLineHeight)];
        _indicateView.backgroundColor = self.style.bottomLineColor;
    }
...
- (void)initItemWidthArray{
    CGFloat originX = self.style.titleMargin;
    if (!self.style.isTitleScrollEnable) {
        ...
        CGFloat itemWidth = (width - ((count+ 1) * self.style.titleMargin))/count;
        for (int i = 0; i < self.itemArray.count; i++) {
            ...
            originX = originX + itemWidth + self.style.titleMargin;
        }
        ...
        //可以滚动
        for (int i = 0; i< self.itemArray.count; i++) {
            CategoriesModel *model = self.itemArray[i];
            NSString *item = model.name;
            NSDictionary *attributes = @{NSFontAttributeName : [UIFont systemFontOfSize:self.style.titleFontSize]};
            CGRect rect = [item boundingRectWithSize:CGSizeMake(MAXFLOAT, self.style.titleViewHeight) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:attributes context:nil];
            //        CGSize textSize = [item sizeWithAttributes:attributes];
            CGFloat currentOriginX = originX;
            [self.itemOriginXArray addObject:@(currentOriginX)];
            NSLog(@"---%f",currentOriginX);
            [self.itemWidthArray addObject:@(rect.size.width)];
            NSLog(@"----%f",rect.size.width);
            originX = originX + rect.size.width + self.style.titleMargin;
        }
...
- (UICollectionViewFlowLayout *)flowLayout
{
    if (!_flowLayout) {
        ...
        _flowLayout.minimumLineSpacing = self.style.titleMargin;
        _flowLayout.minimumInteritemSpacing = self.style.titleMargin;
        _flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        _flowLayout.sectionInset = UIEdgeInsetsMake(0, self.style.titleMargin, 0, self.style.titleMargin);
    }
    return _flowLayout;
}
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CategoriesModel *model = self.itemArray[indexPath.row];
    cell.contentLabel.text = model.name;
    cell.contentLabel.font = [UIFont systemFontOfSize:self.style.titleFontSize];
    cell.index = indexPath.row;
    if (indexPath.row == self.currentIndex) {
        cell.contentLabel.textColor = self.style.titleSelectedColor;
    }
    else{
        cell.contentLabel.textColor = self.style.titleColor;
    }
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNotification *notification = [[NSNotification alloc]initWithName:RESETCOLORNOTIFICATION object:@"color" userInfo:@{@"color":self.style.titleColor,@"index":@(indexPath.row)}];
    [[NSNotificationCenter defaultCenter]postNotification:notification];
...
- (void)adjustContentOffsetToIndex:(NSInteger)index completeHanle:(void(^)())completeHandle animation:(BOOL)animation{
    CGFloat maxContentOffsetX = [[self.itemOriginXArray lastObject]floatValue] + [[self.itemWidthArray lastObject] floatValue] + self.style.titleMargin - self.frame.size.width;
    if (index >= self.itemArray.count||index < 0) {
        return;
    }
